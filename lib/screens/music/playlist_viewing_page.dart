...
class PlaylistViewingPage extends StatefulWidget {
  ...
  const PlaylistViewingPage({super.key, required this.playlist});
...
class _PlaylistViewingPageState extends State<PlaylistViewingPage> with TickerProviderStateMixin {
  ...
  Widget build(BuildContext context) {
    return Consumer<MusicController>(
        builder: (context, musicContoller, _) {
          ...
          final songs = _playlistService.getSongs(widget.playlist.id);
          final playableSongs = mapPlaylistSongsToSongs(songs);
            ...
            children: [
              Positioned.fill(
                child: CustomScrollView(
                  ...
                  slivers: [
                    _buildSliverHeader(musicController: musicContoller, playlist:  widget.playlist,isPlayingFromPlaylist: isPlayingFromPlaylist),
                    SliverToBoxAdapter(child: SizedBox(height: 10,)),
                    _buildSliverSongList(controller: musicContoller, playableSongs: playableSongs, songs: songs),
  ...
  Widget _buildSliverHeader({required MusicController musicController, required Playlist playlist, required bool isPlayingFromPlaylist}) {
    return SliverAppBar(
                ...
                Opacity(
                  ...
                  child: Center(
                    child: Padding(
                      ...
                      child: _buildAlbumCard(isPlayingFromPlaylist: isPlayingFromPlaylist, musicController: musicController),
                    ),
                  ),
                ),
                Positioned(
  ...
  List<Song> mapPlaylistSongsToSongs(List<PlaylistSong> items) {
    return items
        .map((ps) => Song(
      title: ps.title,
  ...
  Widget _buildSliverSongList(
      {
        ...
        required List<Song> playableSongs
      }) {
    if (songs.isEmpty) {
  ...
  Widget _buildAlbumCard({required bool isPlayingFromPlaylist, required MusicController musicController}) {

    return Transform.rotate(
      angle: 0.02,
      child: isPlayingFromPlaylist && musicController.currentSong?.albumArt != null
          ? SizedBox(
        height: 200,
        width: 300,
        child: Stack(
