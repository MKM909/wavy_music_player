...
class AlbumViewingPage extends StatefulWidget {

  final Album album;

  const AlbumViewingPage({super.key, required this.album});

  @override
  State<AlbumViewingPage> createState() => _AlbumViewingPageState();
}

class _AlbumViewingPageState extends State<AlbumViewingPage> with TickerProviderStateMixin {
  ...
  Widget build(BuildContext context) {
    return Consumer<MusicController>(
        builder: (context, musicContoller, _) {

          final current = musicContoller.currentSong;
          final firstSong = widget.album.songs.first;

          final isPlayingFromAlbum =
              current != null &&
                  widget.album.songs.contains(current);

          final songs = widget.album.songs;
              ...
              children: [
                Positioned.fill(
                  child: CustomScrollView(
                    ...
                    slivers: [
                      _buildSliverHeader(musicController: musicContoller, album:  widget.album,isPlayingFromAlbum: isPlayingFromAlbum),
                      SliverToBoxAdapter(child: SizedBox(height: 10,)),
                      _buildSliverSongList(controller: musicContoller, songs: songs,),
                            ...
                            child: GestureDetector(
                              onTap: () {
                                ...
                                if (isPlayingFromAlbum) {
                                  musicContoller.togglePlayPause();
                                } else {
                                  musicContoller.playPlaylist(
                                    songs: songs,
                                    startIndex: 0,
                                  );
                                }
  ...
  Widget _buildSliverHeader({required MusicController musicController, required Album album, required bool isPlayingFromAlbum}) {
    return SliverAppBar(
                ...
                Opacity(
                  ...
                  child: Center(
                    child: Padding(
                      ...
                      child: _buildAlbumCard(isPlayingFromAlbum: isPlayingFromAlbum, musicController: musicController),
                    ),
                  ),
                ),
                Positioned(
  ...
  Widget _buildAlbumCard({required bool isPlayingFromAlbum, required MusicController musicController}) {

    return Transform.rotate(
      angle: 0.02,
      child: isPlayingFromAlbum && musicController.currentSong?.albumArt != null
          ? SizedBox(
        height: 200,
        width: 300,
        child: Stack(
